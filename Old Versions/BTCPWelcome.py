...
class WelcomeScreen:
    def __init__(self, parent):
        ...
        window_width = 600  # Etwas schmaler
        window_height = 400  # Etwas kompakter
...
# ====== THREADED API FUNCTIONS MIT LOADING STATUS ======
def fetch_bitcoin_price_thread():
    """Holt Bitcoin-Preis in einem separaten Thread"""
    try:
        if CURRENCY == "USD":
...
if __name__ == "__main__":
    ...
    # Initialize main window (NICHT versteckt - wir starten es nur nicht im sichtbaren Bereich)
    root = tk.Tk()
    root.overrideredirect(1)
    root.geometry("640x450")
    root.config(bg="#212121")
    
    # WICHTIG: Setze Fensterposition AUSSERHALB des Bildschirms während des Ladens
    root.geometry("+5000+5000")  # Positioniere außerhalb des sichtbaren Bereichs
    
    # CRITICAL: Erstelle ALLE UI-Elemente JETZT (während Fenster "unsichtbar" ist)
    # ====== UI ELEMENTS ======
    # Price Label
    ...
    # Jetzt erst das Fenster "verstecken" (es ist schon komplett gerendert)
    root.withdraw()
    ...
    def show_main_window():
        # CRITICAL: Hole die richtige Position
        last_x, last_y = load_window_position()
        if last_x is not None and last_y is not None:
            # Setze Position aber noch NICHT sichtbar
        ...
        # WICHTIG: Nochmal alles zeichnen lassen an der neuen Position
        root.update_idletasks()
        root.update()
